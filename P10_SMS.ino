...
void scrollScreen() {
  ...
  // Update scroll; Range 0.01 - 0.1; Change to alter scroll speed
...
void handleSms() {
  if (sms.available() > 0) {
    ...
    int i = 0;
    PORTD = (1 << PD3);
    while (sms.available() > 0 && i < (MAX_LEN - 1)) {
      n_char[i++] = sms.read();
      _delay_ms(4);  //wait for the next byte, if after this nothing has arrived it means the text was not part of the same stream entered by the user
    }
    n_char[i] = '\0';
    String new_msg = String(n_char);
